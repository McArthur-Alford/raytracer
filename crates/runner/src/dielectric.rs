...
pub struct DielectricPhase(Material);

impl DielectricPhase {
    pub fn new(
        device: &wgpu::Device,
        ...
        let compute_shader = device
            .create_shader_module(include_spirv!(concat!(env!("OUT_DIR"), "/dielectric.spv")));

        let data_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label,
            contents: bytemuck::cast_slice(&dielectric_data),
